...
(() => {
  ...
  function createExternalModules(onResolved) {
    ...
    function ensureBabel() {
      ...
      babelLoading = loadScriptChain([
        { src: BABEL_URL, integrity: BABEL_SRI },
        { src: 'https://cdn.jsdelivr.net/npm/@babel/standalone@7.29.0/babel.min.js' }
      ]);
      return babelLoading;
    }
  ...
  function loadScriptChain(sources) {
    // Try each source in order; only reject once all have failed. Each
    // "source" is {src, integrity?} — integrity is only safe to keep on
    // sources we've verified (same exact file), so CDN mirrors after the
    // first are listed without one.
    let i = 0;
    const tryNext = () => {
      if (i >= sources.length) return Promise.reject(new Error('all sources failed: ' + sources.map(s => s.src).join(', ')));
      const s = sources[i++];
      return loadScript(s.src, s.integrity).catch(() => tryNext());
    };
    return tryNext();
  }
  function loadReactUmd() {
    ...
    // Self-hosted React is verified byte-for-byte against the SRI hash above
    // (sha384-DGyLxAyjq0f9SPpVevD6IgztCFlnMF6oW/XQGmfe+IsZ8TqEiDrcHkMLKI6fiB/Z),
    // so it's listed first with that same integrity check still enforced.
    // ReactDOM/Babel are too large to safely self-host without risking a
    // subtly-corrupt file, so those keep the CDN chain with unpkg's SRI-pinned
    // copy first and an un-pinned jsdelivr mirror as a fallback if unpkg is
    // ever down (see https://github.com/tradingview/lightweight-charts/issues/1559
    // for why a single CDN dependency is a real, documented failure mode).
    return Promise.all([
      loadScriptChain([
        { src: './react.production.min.js', integrity: REACT_SRI },
        { src: REACT_URL, integrity: REACT_SRI },
        { src: 'https://cdn.jsdelivr.net/npm/react@18.3.1/umd/react.production.min.js' }
      ]),
      loadScriptChain([
        { src: REACT_DOM_URL, integrity: REACT_DOM_SRI },
