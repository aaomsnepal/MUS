...
/* ── PWA install + auto-update ──────────────────────────────────────
   FIX (Sep 2026): this app was never actually registering its service
   worker, so any phone that had installed an earlier build kept a stale
   worker running with no way to get today's edits. This registers it
   properly and forces any waiting update to take over immediately
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", async () => {
    try {
      ...
      reg.update();
    } catch (e) { /* not fatal — app still works without offline support */ }
  });
